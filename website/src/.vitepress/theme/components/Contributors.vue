...
const contributors = computed(() => {
  const list = [...body.value.matchAll(/(?<=\(|(, ))@(.*?)(?=\)|(, ))/g)].map(
    (match) => match[2],
  );
  const uncredited = author.value.includes("[bot]") ? [] : [author.value];

  return [...new Set([...uncredited, ...list])].filter(
    (user) => !nonExistent.value.includes(user),
  );
});
...
function addToNonExistent(user: string) {
  if (!nonExistent.value.includes(user)) nonExistent.value.push(user);
}
</script>
